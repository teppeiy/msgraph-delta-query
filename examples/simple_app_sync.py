...
async def simple_app_sync():
    ...
    try:
        ...
        apps, delta_link, metadata = await client.delta_query_all(
            resource="applications", select=["id", "displayName", "appId"], top=50
        )

        print(
            f"✅ Retrieved {len(apps)} applications in {metadata.duration_seconds:.2f}s"
        )
        ...
        if len(apps) > 0:
            ...
            for i, app in enumerate(apps[:5]):
                ...
                if removed_info:
                    ...
                    print(
                        f"   ✨ {i+1}. {app.get('displayName', 'N/A')} (AppID: {app.get('appId', 'N/A')})"
                    )

...
async def simple_example():
    ...
    try:
        ...
        users, delta_link, metadata = await client.delta_query_all(
            resource="users", select=["id", "displayName", "mail"], top=20
        )
        ...
        if len(users) > 0:
            ...
            for i, user in enumerate(users[:3]):
                print(
                    f"   {i+1}. {user.get('displayName', 'N/A')} - {user.get('mail', 'N/A')}"
                )
            if len(users) > 3:
        ...
        if len(users) > 0:
            ...
            for i, user in enumerate(users[:5]):
                ...
                if removed_info:
                    ...
                    print(
                        f"   {status} {i+1}. {user.get('displayName', 'N/A')} - {user.get('mail', 'N/A')}"
                    )
...
async def persistent_applications_sync():
    ...
    try:
        ...
        async for apps, page_metadata in client.delta_query_stream(
            ...
            print(
                f"📦 Batch {batch_count}: {len(apps)} applications (Page {page_metadata.page})"
            )
            print(
                f"   Changes: {page_metadata.page_new_or_updated} new/updated, "
            ...
            if apps:
                ...
                if removed_info:
                    ...
                    print(
                        f"   ✨ Sample: [NEW/UPDATED] {app.get('displayName', 'N/A')} (AppID: {app.get('appId', 'N/A')})"
                    )
        ...
        for resource_name, select_fields in resources:
            ...
            try:
                objects, delta_link, metadata = await client.delta_query_all(
                    resource=resource_name, select=select_fields, top=50
                )

                print(
                    f"   ✅ {len(objects)} {resource_name} synced in {metadata.duration_seconds:.2f}s"
                )
                print(f"   📊 {metadata.change_summary}")

                if len(objects) == 0:
                    print(
                        f"   📭 No changes for {resource_name} (delta query optimized!)"
                    )
...
async def demonstrate_storage_persistence():
    ...
    try:
        users1, _, metadata1 = await client1.delta_query_all(
            resource="users", select=["id", "displayName"], top=10
        )
    ...
    try:
        ...
        if len(users2) == 0:
            print(
                "   🎯 Perfect! No changes detected - delta link was used successfully!"
            )
        else:
...
async def main():
    ...
    if missing_vars:
        print(
            f"❌ Missing required Microsoft Graph environment variables: {', '.join(missing_vars)}"
        )
    ...
    connection_string = os.getenv("AZURE_STORAGE_CONNECTION_STRING") or os.getenv(
        "AzureWebJobsStorage"
    )
    ...
    print(
        "   🔧 Development: Uses connection string (set AZURE_STORAGE_CONNECTION_STRING)"
    )

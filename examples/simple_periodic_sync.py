...
async def run_user_sync():
    ...
    print(
        f"🔄 Starting sync at {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}"
    )
    ...
    try:
        ...
        print(
            f"   📈 Processed {metadata.pages_fetched} pages, {len(users)} total users"
        )
        ...
        if len(users) > 0:
            ...
            for i, user in enumerate(users[:3]):
                ...
                print(
                    f"     {i+1}. {status} {user.get('displayName', 'N/A')} (created: {created})"
                )
            if len(users) > 3:

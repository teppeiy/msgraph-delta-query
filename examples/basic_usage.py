        ...
        for i, app in enumerate(apps[:5]):  # Show more examples
            ...
            if removed_info:
                ...
                status = (
                    "DELETED"
                    if reason == "deleted"
                    else "SOFT DELETED" if reason == "changed" else f"REMOVED-{reason}"
                )
                print(f"  {i+1}. [{status}] ID: {app.get('id', 'N/A')}")
            else:
                print(
                    f"  {i+1}. [NEW/UPDATED] {app.get('displayName', 'N/A')} (AppID: {app.get('appId', 'N/A')})"
                )
...
async def streaming_example():
    ...
    try:
        ...
        async for apps, page_metadata in client.delta_query_stream(
            resource="applications", select=["id", "displayName", "appId"], top=25
        ):
            batch_count += 1
            total_apps += len(apps)
            ...
            print(
                f"  Page changes: {page_metadata.page_new_or_updated} new/updated, "
...
async def periodic_sync_example():
    ...
    try:
        while True:
            sync_count += 1
            print(
                f"🔄 Starting sync #{sync_count} at {asyncio.get_event_loop().time():.0f}s"
            )
            ...
            try:
                ...
                print(
                    f"   Retrieved {len(users)} users in {user_meta.duration_seconds:.2f}s"
                )

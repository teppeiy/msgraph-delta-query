...
class TestExecuteDeltaRequestMethod:
    ...
    async def test_execute_delta_request_success(self, mock_credential):
        ...
        mock_request_builder.DeltaRequestBuilderGetRequestConfiguration = mock_config_class
    ...
    async def test_execute_delta_request_delta_error_with_fallback(self, mock_credential, mock_storage):
        ...
        with patch('logging.warning') as mock_warning:
            with patch('logging.info') as mock_info:
                ...
                mock_info.assert_called_with("Clearing invalid stored delta link for users")
    ...
    async def test_execute_delta_request_fallback_also_fails(self, mock_credential, mock_storage):
        ...
        with patch('logging.error') as mock_error:
            with pytest.raises(Exception, match="Fallback also failed"):
                ...
                mock_error.assert_called_with("Fallback to full sync also failed: Fallback also failed")
...
class TestDeltaQueryStreamMethod:
    ...
    async def test_delta_query_stream_with_stored_delta_link(self, mock_credential, mock_storage):
        """Test delta_query_stream using stored delta link."""
        client = AsyncDeltaQueryClient(credential=mock_credential, delta_link_storage=mock_storage)
        
        # Set up stored delta link and metadata
        stored_delta_link = "https://graph.microsoft.com/v1.0/users/delta?$deltatoken=stored123"
        metadata = {
        ...
        with patch.object(client, '_get_delta_request_builder') as mock_builder:
            with patch.object(client, '_extract_delta_token_from_link', return_value="stored123") as mock_extract:
                with patch('logging.info') as mock_info:
                    ...
                    assert any("Using stored delta link for users incremental sync" in call for call in info_calls)
    ...
    async def test_delta_query_stream_with_explicit_delta_link(self, mock_credential):
        ...
        explicit_delta_link = "https://graph.microsoft.com/v1.0/users/delta?$deltatoken=explicit123"
        
        await client._initialize()
        
        with patch.object(client, '_get_delta_request_builder') as mock_builder:
            with patch.object(client, '_extract_delta_token_from_link', return_value="explicit123") as mock_extract:
                mock_builder.return_value = Mock()
                
                try:
                    # Start the async generator
                    stream = client.delta_query_stream("users", delta_link=explicit_delta_link)
                    await stream.__anext__()
                except StopAsyncIteration:
                    pass
                except Exception:
...
class TestClientInitializationEdgeCases:
    ...
    async def test_signal_handler_setup_os_error(self):
        ...
        with patch('asyncio.get_running_loop') as mock_get_loop:
            mock_loop = Mock()
            mock_loop.add_signal_handler = Mock(side_effect=OSError("Signals not supported"))
            mock_get_loop.return_value = mock_loop
    ...
    async def test_signal_handler_setup_not_implemented_error(self):
        ...
        with patch('asyncio.get_running_loop') as mock_get_loop:
            mock_loop = Mock()
            mock_loop.add_signal_handler = Mock(side_effect=NotImplementedError("Not implemented"))
            mock_get_loop.return_value = mock_loop

...
class TestAsyncDeltaQueryClientExtendedCoverage:
    ...
    async def test_get_delta_request_builder_all_resources(self):
        ...
        assert client._get_delta_request_builder("users") == mock_graph_client.users.delta
        assert client._get_delta_request_builder("Users") == mock_graph_client.users.delta
        assert client._get_delta_request_builder("applications") == mock_graph_client.applications.delta
        assert client._get_delta_request_builder("groups") == mock_graph_client.groups.delta
        assert client._get_delta_request_builder("serviceprincipals") == mock_graph_client.service_principals.delta
        assert client._get_delta_request_builder("servicePrincipals") == mock_graph_client.service_principals.delta
    ...
    async def test_extract_delta_token_from_link_malformed_url(self):
        ...
        with patch('msgraph_delta_query.client.urllib.parse.urlparse', side_effect=Exception("Parse error")):
    ...
    async def test_initialization_with_azure_blob_storage_connection_string_logging(self):
    ...
    async def test_signal_handler_setup_with_running_loop(self):
        ...
        with patch('asyncio.get_running_loop') as mock_get_loop:
            ...
            assert mock_loop.add_signal_handler.call_count >= 0  # May be called for SIGTERM and SIGINT
    ...
    async def test_destructor_cleanup_warning(self):
        ...
        with patch('asyncio.get_running_loop', side_effect=RuntimeError("No running loop")):
            with patch('msgraph_delta_query.client.logging.warning') as mock_warning:
                ...
                mock_warning.assert_called_with('AsyncDeltaQueryClient destroyed without proper cleanup (no running event loop)')

...
class TestPaginationBugRegression:
    ...
    @pytest.mark.asyncio
    async def test_pagination_import_path_verification(self, mock_credential, mock_storage):
        ...
        first_response.odata_next_link = "https://graph.microsoft.com/v1.0/users/delta?$skiptoken=page2"
        ...
        second_response.odata_delta_link = "https://graph.microsoft.com/v1.0/users/delta?deltatoken=final"
        ...
        async for objects, page_meta in client.delta_query_stream("users"):
            ...
            for obj in objects:
                ...
                assert obj.odata_type == "#microsoft.graph.user", f"Expected user object, got {obj.odata_type}"
    ...
    @pytest.mark.asyncio
    async def test_applications_vs_users_response_type_isolation(self, mock_credential, mock_storage):
        ...
        test_scenarios = [
            ...
            ("applications", "#microsoft.graph.application", "applications.delta.delta_get_response"),
        ]
        
        for resource_type, expected_odata_type, expected_module_suffix in test_scenarios:
            ...
            async for objects, page_meta in client.delta_query_stream(resource=resource_type):
                for obj in objects:
    ...
    async def test_bug_reproduction_attempt(self, mock_credential, mock_storage):
        ...
        async def track_pagination(request_info, response_type, error_map):
            ...
            page2_response.odata_delta_link = "https://graph.microsoft.com/v1.0/users/delta?deltatoken=final"
        ...
        assert "#microsoft.graph.application" not in all_types  # Bug would have caused this
        
        # Verify pagination used correct response type
        assert len(pagination_calls) == 1
        assert "users.delta.delta_get_response" in pagination_calls[0]['response_type_module']
        assert "applications.delta.delta_get_response" not in pagination_calls[0]['response_type_module']

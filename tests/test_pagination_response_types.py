...
class TestPaginationResponseTypes:
    ...
    def create_mock_user_response(self, user_count: int = 5, has_next: bool = False):
        ...
        mock_response.odata_next_link = "https://graph.microsoft.com/v1.0/users/delta?$skiptoken=next_page_token" if has_next else None
        mock_response.odata_delta_link = None if has_next else "https://graph.microsoft.com/v1.0/users/delta?deltatoken=final_token"
        
        return mock_response

    def create_mock_application_response(self, app_count: int = 5, has_next: bool = False):
        ...
        mock_response.odata_next_link = "https://graph.microsoft.com/v1.0/applications/delta?$skiptoken=next_page_token" if has_next else None
        mock_response.odata_delta_link = None if has_next else "https://graph.microsoft.com/v1.0/applications/delta?deltatoken=final_token"
        
        return mock_response

    @pytest.mark.asyncio
    async def test_users_pagination_uses_correct_response_type(self, mock_credential, mock_storage):
        ...
        assert captured_response_type.__module__.endswith('users.delta.delta_get_response')

    @pytest.mark.asyncio
    async def test_applications_pagination_uses_correct_response_type(self, mock_credential, mock_storage):
        ...
        # Create mock responses for pagination
        first_response = self.create_mock_application_response(app_count=3, has_next=True)
        second_response = self.create_mock_application_response(app_count=2, has_next=False)
        ...
        async for objects, page_meta in client.delta_query_stream(resource="applications"):
            pages_processed += 1
        ...
        # Verify the correct response type was used for pagination
        assert captured_response_type is not None
        assert captured_response_type.__module__.endswith('applications.delta.delta_get_response')

    @pytest.mark.asyncio
    async def test_stored_delta_link_uses_correct_response_type(self, mock_credential, mock_storage):
        """Test that stored delta link usage also uses the correct response type."""
        client = AsyncDeltaQueryClient(credential=mock_credential, delta_link_storage=mock_storage)
        
        # Mock storage to return a stored delta link
        stored_delta_link = "https://graph.microsoft.com/v1.0/users/delta?deltatoken=stored_token"
        mock_storage.get = AsyncMock(return_value=(stored_delta_link, {"metadata": "test"}))
    ...
    @pytest.mark.asyncio
    async def test_mixed_object_types_bug_prevention(self, mock_credential, mock_storage):
    ...
    async def test_unsupported_resource_type_error(self, mock_credential, mock_storage):
        ...
        first_response.odata_next_link = "https://graph.microsoft.com/v1.0/unsupported/delta?$skiptoken=token"
        ...
        with pytest.raises(ValueError, match="Unsupported resource type"):
            async for objects, page_meta in client.delta_query_stream(resource="unsupported"):

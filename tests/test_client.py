...
from msgraph_delta_query.client import AsyncDeltaQueryClient, _cleanup_all_clients, _client_registry
...
class TestAsyncDeltaQueryClient:
    ...
    async def test_initialize_creates_session_and_credential(self):
        ...
        with patch('msgraph_delta_query.client.aiohttp.ClientSession') as mock_session_class:
    ...
    async def test_make_request_timeout_retry(self, mock_credential):
        ...
        async def mock_get_side_effect(*args, **kwargs):
            ...
            if call_count == 1:
                ...
                context_manager = AsyncMock()
                context_manager.__aenter__ = AsyncMock(return_value=mock_response_success)
                context_manager.__aexit__ = AsyncMock(return_value=None)
                return context_manager
    ...
    async def test_delta_query_stream_with_parameters(self, mock_credential, mock_storage):
        ...
        with patch.object(client, 'get_token', return_value="test_token"):
            with patch.object(client, '_make_request') as mock_request:
                ...
                assert "select=id%2CdisplayName" in called_url or "$select=id,displayName" in called_url
                assert "filter=startswith" in called_url  # Just check for the function name
                assert "top=100" in called_url or "$top=100" in called_url
                assert "deltatoken=latest" in called_url or "$deltatoken=latest" in called_url

    async def test_delta_query_stream_with_existing_delta_link(self, mock_credential, mock_storage):
        ...
        with patch.object(client, 'get_token', return_value="test_token"):
            with patch.object(client, '_make_request') as mock_request:
                ...
                called_url = mock_request.call_args[0][0]
                assert "deltatoken=stored_token" in called_url or "$deltatoken=stored_token" in called_url
...
async def test_delta_query_stream_saves_final_delta_link(mock_credential, mock_storage):
    ...
    with patch.object(client, 'get_token', return_value="test_token"):
        with patch.object(client, '_make_request') as mock_request:
            ...
            pages = []
            async for objects, page_meta in client.delta_query_stream("users", select=["id", "name"]):
                pages.append((objects, page_meta))
...
@pytest.mark.asyncio 
async def test_delta_query_all_metadata_used_stored_deltalink(mock_credential, mock_storage):
...
@pytest.mark.asyncio
async def test_delta_query_all_metadata_no_stored_deltalink(mock_credential, mock_storage):

...
class ResourceWarningCapture:
    ...
    def warning_handler(self, message, category, filename, lineno, file=None, line=None):
        ...
        if category == ResourceWarning and "Unclosed" in str(message):
            self.warnings.append(warnings.WarningMessage(
                category, message, filename, lineno, file, line
            ))
...
class TestUnclosedSessionBugFixes:
    ...
    async def test_explicit_close_prevents_unclosed_sessions(self, mock_storage):
        ...
        unclosed_warnings = [w for w in capture.warnings if "client session" in str(w.message).lower()]
        assert len(unclosed_warnings) == 0, f"Found unclosed session warnings: {[str(w.message) for w in unclosed_warnings]}"
    ...
    async def test_multiple_close_calls_safe(self, mock_storage):
        ...
        assert len(capture.warnings) == 0, f"Unexpected warnings from multiple close calls: {[str(w.message) for w in capture.warnings]}"
    ...
    async def test_context_manager_with_exception_cleanup(self, mock_storage):
        ...
        unclosed_warnings = [w for w in capture.warnings if "client session" in str(w.message).lower()]
        assert len(unclosed_warnings) == 0, f"Found unclosed session warnings after exception: {[str(w.message) for w in unclosed_warnings]}"
    ...
    async def test_local_file_storage_cleanup(self):
        ...
        unclosed_warnings = [w for w in capture.warnings if "client session" in str(w.message).lower()]
        assert len(unclosed_warnings) == 0, f"Found unclosed session warnings with LocalFileDeltaLinkStorage: {[str(w.message) for w in unclosed_warnings]}"

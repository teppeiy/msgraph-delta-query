...
class TestIntegration:
    ...
    async def test_end_to_end_delta_query_with_storage(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            client = AsyncDeltaQueryClient(
                credential=mock_credential, delta_link_storage=storage
            )
            ...
            with patch.object(client, "_make_request") as mock_request:
                ...
                assert (
                    delta_link
                    == "https://graph.microsoft.com/v1.0/users/delta?$deltatoken=first_token"
                )
            ...
            with patch.object(client2, "_make_request") as mock_request2:
                ...
                assert (
                    delta_link2
                    == "https://graph.microsoft.com/v1.0/users/delta?$deltatoken=second_token"
                )
                ...
                assert (
                    "deltatoken=first_token" in called_url
                )  # Works with both encoded and unencoded
    ...
    async def test_concurrent_clients_same_storage(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            client1 = AsyncDeltaQueryClient(
                credential=mock_credential1, delta_link_storage=storage
            )
            client2 = AsyncDeltaQueryClient(
                credential=mock_credential2, delta_link_storage=storage
            )
            ...
            with patch.object(
                client1,
                "_make_request",
                return_value=(200, json.dumps(response1), response1),
            ):
                with patch.object(
                    client2,
                    "_make_request",
                    return_value=(200, json.dumps(response2), response2),
                ):
            ...
            assert (
                await storage.get("users")
                == "https://example.com/users/delta?token=users_token"
            )
            assert (
                await storage.get("groups")
                == "https://example.com/groups/delta?token=groups_token"
            )
    ...
    async def test_client_reset_delta_link_integration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert (
                await storage.get("users")
                == "https://example.com/delta?token=old_token"
            )
    ...
    async def test_storage_metadata_persistence(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(
                client,
                "_make_request",
                return_value=(200, json.dumps(response), response),
            ):
            ...
            with open(path, "r") as f:
                ...
                assert (
                    stored_data["delta_link"]
                    == "https://example.com/delta?token=test_token"
                )
    ...
    async def test_error_handling_integration(self):
        ...
        invalid_storage = LocalFileDeltaLinkStorage(
            folder="/invalid/path/that/does/not/exist"
        )
        client = AsyncDeltaQueryClient(
            credential=mock_credential, delta_link_storage=invalid_storage
        )

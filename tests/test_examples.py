...
async def test_example_usage():
    ...
    resource_params = ResourceParams(
        select=["id", "displayName", "mail"], filter=None, top=100
    )
    ...
    with patch(
        "msgraph_delta_query.client.AsyncDeltaQueryClient", return_value=mock_client
    ):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch.object(client, "_make_request") as mock_request:
            mock_request.return_value = (
                200,
                json.dumps(initial_response),
                initial_response,
            )

            users, delta_link, meta = await client.delta_query_all(
                "users", select=["id", "displayName", "mail"], top=1000
            )
        ...
        with patch.object(client2, "_make_request") as mock_request2:
            mock_request2.return_value = (
                200,
                json.dumps(incremental_response),
                incremental_response,
            )
            ...
            assert (
                "deltatoken=initial_token" in called_url
...
async def test_real_world_scenario_large_dataset_pagination():
    ...
    page1_response = {
        "value": [
            {"id": f"user{i}", "displayName": f"User {i}"} for i in range(1, 101)
        ],
    ...
    page2_response = {
        "value": [
            {"id": f"user{i}", "displayName": f"User {i}"} for i in range(101, 201)
        ],
    ...
    page3_response = {
        "value": [
            {"id": f"user{i}", "displayName": f"User {i}"} for i in range(201, 251)
        ],
    ...
    with patch.object(client, "_make_request") as mock_request:
        ...
        async for users, page_meta in client.delta_query_stream("users", top=100):
            ...
            if page_count < 3:
                ...
                assert (
                    page_meta.delta_link
                    == "https://graph.microsoft.com/v1.0/users/delta?$deltatoken=final_token"
                )
...
async def test_real_world_scenario_concurrent_requests():
    ...
    client = AsyncDeltaQueryClient(
        credential=mock_credential, max_concurrent_requests=2
    )
    ...
    with patch.object(client, "_make_request") as mock_request:
        ...
        results = await asyncio.gather(
            client.delta_query_all("users"), client.delta_query_all("groups")
        )

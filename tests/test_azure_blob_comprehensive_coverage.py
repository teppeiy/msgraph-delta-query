...
class TestAzureBlobStorageComprehensiveCoverage:
    ...
    async def test_priority_2_azure_storage_connection_string_env(self):
        ...
        with patch.dict(os.environ, {"AZURE_STORAGE_CONNECTION_STRING": test_conn}, clear=True):
            storage = AzureBlobDeltaLinkStorage()
    ...
    async def test_priority_3_local_settings_json_with_azure_webjobs_storage(self):
        ...
        local_settings = {
            "Values": {
                "AzureWebJobsStorage": test_conn
            }
        }
        
        # Create a temporary local.settings.json
        with tempfile.NamedTemporaryFile(mode='w', suffix='local.settings.json', delete=False) as f:
    ...
    async def test_connection_string_account_extraction_error(self):
        ...
        with patch.dict(os.environ, {"AZURE_STORAGE_CONNECTION_STRING": malformed_conn}, clear=True):
            storage = AzureBlobDeltaLinkStorage()
    ...
    async def test_blob_service_client_no_connection_error(self):
        ...
        with pytest.raises(ValueError, match="No account URL or connection string available"):
    ...
    async def test_ensure_container_exists_error(self):
        ...
        mock_container_client.create_container.side_effect = ServiceRequestError("Service unavailable")
        
        with patch.object(storage, '_get_blob_service_client', return_value=mock_blob_service):
    ...
    async def test_get_delta_link_non_string_delta_link(self):
        ...
        mock_download_stream.readall.return_value = json.dumps(invalid_data).encode("utf-8")
        mock_blob_client.download_blob.return_value = mock_download_stream
        mock_blob_service.get_blob_client.return_value = mock_blob_client
    ...
    async def test_get_metadata_general_exception(self):
        ...
        with patch.object(storage, '_ensure_container_exists', side_effect=Exception("General error")):
    ...
    async def test_explicit_connection_string_priority(self):
        ...
        explicit_conn = "DefaultEndpointsProtocol=https;AccountName=explicit;AccountKey=key123"
        env_conn = "DefaultEndpointsProtocol=https;AccountName=envaccount;AccountKey=key456"
